...
import static com.github.rebue.hlw.mapper.HlwStudentDynamicSqlSupport.*;
    ...
    * @mbg.generated 自动生成，如需修改，请删除本行
    ...
    default Optional<HlwStudentMo> selectOne(HlwStudentMo record) {
        return selectOne(c ->
            ...
            .and(studentCode, isEqualTo(record::getStudentCode).when(Objects::nonNull))
            .and(name, isEqualTo(record::getName).when(Objects::nonNull))
            .and(phone1, isEqualTo(record::getPhone1).when(Objects::nonNull))
            .and(phone2, isEqualTo(record::getPhone2).when(Objects::nonNull))
            .and(idCard, isEqualTo(record::getIdCard).when(Objects::nonNull))
            .and(age, isEqualTo(record::getAge).when(Objects::nonNull))
            .and(birthday, isEqualTo(record::getBirthday).when(Objects::nonNull))
            .and(sex, isEqualTo(record::getSex).when(Objects::nonNull))
            .and(height, isEqualTo(record::getHeight).when(Objects::nonNull))
            .and(weight, isEqualTo(record::getWeight).when(Objects::nonNull))
            .and(isCpc, isEqualTo(record::getIsCpc).when(Objects::nonNull))
            .and(regDatetime, isEqualTo(record::getRegDatetime).when(Objects::nonNull))
            .and(communicateTime, isEqualTo(record::getCommunicateTime).when(Objects::nonNull))
        );
    }
